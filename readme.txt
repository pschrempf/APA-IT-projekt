=== APA Annotation Plugin ===
Contributors: (this should be a list of wordpress.org userid's)
Tags: annotation, service, press
Requires at least: 4.0.1
Tested up to: 4.2.3
Stable tag: trunk
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Using this plugin you can annotate your posts using the Austrian Press Agency annotation service.

== Description ==

This annotation service uses the Austrian Press Agency's Power Search to deliver you possible annotation results.

You will be presented with these results and will have the possibility to select suitable annotations to be saved.
These annotations can then be shown on your website on their own landing pages.

== Installation ==

1. Download the plugin
2. Place the folder into the `/wp-content/plugins/` directory.
3. Activate the plugin through the 'Plugins' menu in WordPress.
4. Various pages have now been added to your site, please go to the settings menu to set up the plugin.
5. To make the plugin fit a custom theme you may have to edit the 'annotations-template.php' and 'annotations.css' files.

== Frequently Asked Questions ==

= Are there any other languages available? =

The annotations are only supported in French and German at the moment. The plugin is available in German and English.

= How do I delete annotations? =

Go to the annotations page, select the annotations to be deleted and click the 'delete' button on the bottom of the page. 
Now wait until the desired annotations disappear from the list.

== Changelog ==

